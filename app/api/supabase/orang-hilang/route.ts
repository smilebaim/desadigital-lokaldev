...
import { KABUPATEN, NAMA_ORANG, rand, kecLabel, UPDATED_AT } from "@/lib/dummy";
export async function GET() {
  const data = Array.from({ length: 24 }, (_, i) => ({
    id: `OH-${i + 1}`,
    name: NAMA_ORANG[i % NAMA_ORANG.length],
    nama: NAMA_ORANG[i % NAMA_ORANG.length],
    age: rand(5, 70),
    usia: rand(5, 70),
    ...
    district: kecLabel(i),
    kecamatan: kecLabel(i),
    status: i < 8 ? "dicari" : "ditemukan",
    lastSeen: `2026-04-${String(rand(20, 28)).padStart(2, "0")}`,
    tanggal_hilang: `2026-04-${String(rand(20, 28)).padStart(2, "0")}`,
    description: i < 8 ? "Masih dalam pencarian Tim SAR" : "Berhasil ditemukan dalam kondisi selamat",
    keterangan: i < 8 ? "Masih dalam pencarian Tim SAR" : "Berhasil ditemukan dalam kondisi selamat",
  ...
  return NextResponse.json({ data, summary, total: data.length, updated_at: UPDATED_AT() });
}
